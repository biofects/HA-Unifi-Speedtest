...
class UniFiSpeedTestConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        """Configure a UniFi OS controller using a local API key."""
        errors = {}
        _LOGGER.info("Configuring UniFi OS credentials")
        ...
        if user_input is not None:
            ...
            try:
                ...
                entry_data = {
                    ...
                    CONF_RUN_SPEEDTEST_ON_STARTUP: user_input.get(
                        CONF_RUN_SPEEDTEST_ON_STARTUP,
                        DEFAULT_RUN_SPEEDTEST_ON_STARTUP,
                    ),
                ...
                return self.async_create_entry(
                    title=f"{INTEGRATION_NAME} (UniFi OS)",
                    data=entry_data
                )
                ...
                if "api key is required" in error_str or "api_key" in error_str:
                    ...
                    _LOGGER.error("A local API key is required for UniFi OS")
